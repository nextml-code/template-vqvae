    ...
    def generated(self, n_samples):
        ...
        for w in range(self.feature_shape[-1]):
            for h in range(self.feature_shape[-2]):
                continuous = self.distribution(quantized).sample()
                quantized, *_ = self.quantizer(continuous)
...
class RelativeDecoderBlock(nn.Module):
    ...
    def generated(self, previous):
        ...
        for w in range(self.feature_shape[-1]):
            for h in range(self.feature_shape[-2]):
                continous = self.distribution(previous, quantized).sample()
                quantized, *_ = self.quantizer(continous)
...
class DecoderNVAE(nn.Module):
    def __init__(self, example_features, latent_channels, n_embeddings, level_sizes):
        ...
        self.image = ModuleCompose(
            ...
            Upsample(previous.shape[1], previous.shape[1] // 2),
            DecoderCell(previous.shape[1] // 2),
            Upsample(previous.shape[1] // 2, previous.shape[1] // 4),
            DecoderCell(previous.shape[1] // 4),
            nn.BatchNorm2d(previous.shape[1] // 4),
            nn.Conv2d(previous.shape[1] // 4, 3, kernel_size=1),
            torch.sigmoid,
        )
